...
ROW_NUMBER()over(partition by   A.EC order by C.TP_HABILITADO DESC, C.TP_PREFERENCIAL, C.TP_ORIGEM desc) As QUANTIDADE
FROM ##CARGA_DIA A
...
SET @BCPFILENAME_SMS = '\\10.155.4.51\itf_in\AVON\ARQUIVOS_RAW\AVON_FASE1_QUEBRA_OLOS_ENVIO'+'_'+REPLACE((CONVERT(VARCHAR,CONVERT(DATE,(GETDATE())))),'-','')+'.txt'
