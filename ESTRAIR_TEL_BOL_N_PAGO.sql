...
SELECT DISTINCT *  INTO ##TELEFONES FROM (
SELECT DISTINCT
A.ID_CLIENTE,
A.ID_ACORDO,
C.NM_NOME,
...
WHERE CONVERT(DATE, FORA.VENCIMENTO, 103) between '2020-07-01' AND CONVERT(DATE, GETDATE()-1, 103)
AND STATUS_PROMESSA = 0
AND FASE = '002'
AND TIPO = 'NN'
AND DIR_IND = 'DIRETO'
AND OPERADOR NOT IN ('INDIRETO')
AND FORA.DT_PAGAMENTO IS NULL
AND FORA.DT_CANCELAMENTO IS NULL

IF OBJECT_ID('tempdb..##tel_esco') IS NOT NULL DROP TABLE ##tel_esco
SELECT DISTINCT CPF into ##tel_esco FROM ##TELEFONES
...
INNER JOIN ##tel_esco B ON A.CPF = B.CPF
WHERE A.PRIORIDADE in (1, 2)