...
LEFT JOIN TB_ACORDO_DIVIDA B (NOLOCK) ON B.ID_ACORDO = A.ID_ACORDO
LEFT JOIN TB_CLIENTE C (NOLOCK) ON C.ID_CLIENTE = B.ID_CLIENTE
...
LEFT JOIN (SELECT ID_CONTRATO,ID_ACORDO,NM_DETALHE_VALOR,CASE WHEN NM_DETALHE_VALOR = '002' THEN 'FASE 1' WHEN NM_DETALHE_VALOR = '506' THEN  'FASE 2' ELSE '' END as FASE
		   FROM TB_ACORDO_DIVIDA_DETALHE WHERE ID_DETALHE = 351) FASE ON FASE.ID_CONTRATO = D.ID_CONTRATO

LEFT JOIN (SELECT ID_CONTRATO,ID_ACORDO,NM_DETALHE_VALOR
		   FROM TB_ACORDO_DIVIDA_DETALHE WHERE ID_DETALHE = 121) RISCO ON /*FASACO.ID_ACORDO = A.ID_ACORDO AND*/ RISCO.ID_CONTRATO = D.ID_CONTRATO
