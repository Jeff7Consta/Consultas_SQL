...
IF OBJECT_ID('tempdb..##DESCONHECE') IS NOT NULL DROP TABLE ##DESCONHECE
...
IF OBJECT_ID('tempdb..##ESTRAIR_DESC') IS NOT NULL DROP TABLE ##ESTRAIR_DESC
SELECT* into ##ESTRAIR_DESC FROM ##TUDOTEL A
 INNER JOIN ##DESCONHECE B ON A.TELEFONE = B.TELEFONE2

/*EXPORTA CLIENTES DESCONHECIDOS*/
DECLARE @BCPSQL_EXT VARCHAR(2000),
@BCPFILENAME_EXT VARCHAR(200)
SET @BCPFILENAME_EXT = '\\10.155.4.51\itf_in\AVON\DESCONHECE_CLIENTE\DESCONHECE_CLIENTE_AVON'+'_'+REPLACE((CONVERT(VARCHAR,CONVERT(DATE,(GETDATE())))),'-','')+'.CSV'
SET @BCPSQL_EXT = 'SELECT * FROM ##ESTRAIR_DESC'
...
DECLARE @BCPSQL_RAW VARCHAR(2000),
@BCPFILENAME_RAW VARCHAR(200)
SET @BCPFILENAME_RAW = '\\10.155.4.51\itf_in\AVON\ARQUIVOS_RAW\AVON_FASE1_M0_OLOS_ENVIO'+'_'+REPLACE((CONVERT(VARCHAR,CONVERT(DATE,(GETDATE())))),'-','')+'.txt'
SET @BCPSQL_RAW = 'SELECT * FROM ##RAWOUT'
SET @BCPSQL_RAW = 'BCP "' + @BCPSQL_RAW + '" QUERYOUT ' + @BCPFILENAME_RAW + ' -c -t; -r\n -CP850 -T -Slocalhost'
PRINT @BCPSQL_RAW 
 
DECLARE @RC_RAW VARCHAR(100)
EXEC @RC_RAW = MASTER..XP_CMDSHELL @BCPSQL_RAW
